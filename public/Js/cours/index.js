document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadCours(page) {
      const coursList = document.getElementById("list-cours");
      fetch(`api/cours?page=${page}&limit=${limit}`)
          ...
          if (data.cours && data.cours.length > 0) {
            data.cours.forEach((cours) => {
              ...
              tr.innerHTML = `
                                ...
                                <td class="px-6 py-4">${cours.nom}</td>
                                <td class="px-6 py-4">${cours.module}</td>
                                <td class="px-6 py-4">${cours.professeur}</td>
                                <td class="px-6 py-4">
                              ...
                              `;
              coursList.appendChild(tr);
            });
            updatePagination(data.totalPages, data.currentPage);
          } else {
            coursList.innerHTML = "<tr>Aucun cours trouvé.</tr>";
          }
        })
